...
public class InitializerFactory {
	...
	public ChannelInitializer<SocketChannel> createInitializer(ServerMeta sMeta){
		initialSerializer(sMeta.getAppMeta().getExtSerializer());
		ChannelInitializer<SocketChannel> initializer = null;
		if(sMeta.getProtocol().equals(ProtocolType.HTTP)){
			initializer = new HttpServerInitializer(sMeta);
		}else if(sMeta.getProtocol().equals(ProtocolType.TCP)){
			initializer = new TcpServerInitializer(sMeta);
		}
		return initializer;
	}
