...
public class ServiceRelivePolicy implements RelivePolicy {
	...
	private volatile int threshold = Constants.DEFAULT_RELIVE_THRESHOLD;
	private int SCALE = 2;
	...
	private boolean meetTimePolicy() {
		...
		if (start == 0) {
			...
			if (tmp > interval) {
				start = System.currentTimeMillis();
				v = true;
				logger.warn(Constants.LOG_PROFIX + providerHost + " node revive due to meetTimePolicy:" + interval + "ms");
			}
		}
		return v;
	}
	...
	private boolean meetCountPolicy() {
		...
		if (tryCount >= threshold) {
			tryCount = 0;
			v = true;
			logger.warn(Constants.LOG_PROFIX + providerHost + " node revive due to meetCountPolicy:" + threshold);
		}
		return v;
	}
	...
	public void reset() {
		tryCount = 0;
		threshold = Constants.DEFAULT_RELIVE_THRESHOLD;
		start = 0;
