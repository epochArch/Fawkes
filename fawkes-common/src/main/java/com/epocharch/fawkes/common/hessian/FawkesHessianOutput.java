...
import com.epocharch.fawkes.common.context.InvocationContext;
import com.epocharch.fawkes.common.hessian.io.FawkesExtSerializerFactory;
 ...
 * </pre>
 */
public class FawkesHessianOutput extends AbstractHessianOutput {
	// the output stream/
	 ...
	 */
	public FawkesHessianOutput(OutputStream os) {
		init(os);
	}
	...
	public void init(OutputStream os) {
		...
		if (_serializerFactory == null)
			_serializerFactory = FawkesExtSerializerFactory.createFactory();
	}
	...
	public void startCall(String method) throws IOException {
		...
		os.write(0);
		// add requestId start
		writeHeader(Constants.REQUEST_ID);
		writeString(FawkesContextUtil.getRequestId());
		writeHeader(Constants.GLOBAL_ID);
		writeString(FawkesContextUtil.getGlobalId());
		writeHeader(Constants.TXN_ID);
		writeString(FawkesContextUtil.getTransactionId());
		writeHeader(Constants.INVOKE_TIME);
		writeObject(FawkesContextUtil.getAttribute(Constants.INVOKE_TIME, null));
		writeHeader(Constants.REQUEST_HOP);
		writeObject((FawkesContextUtil.getRequestHop() + 1 + ""));
		writeHeader(Constants.TOKEN_GRAY);
		writeObject(FawkesContextUtil.getAttribute(Constants.TOKEN_GRAY, null));
		writeHeader(PropKey.HOST_IP);
		writeObject(PropertiesContainer.getInstance().getProperty(PropKey.HOST_IP, ""));
		writeHeader(PropKeys.APP_ID);
		writeObject(FawkesContextUtil.getAttribute(PropKeys.APP_ID, "unknowPool"));
		writeHeader(Constants.CLIENT_VERSION);
		writeObject(FawkesContextUtil.getAttribute(Constants.CLIENT_VERSION, "unknowClient"));
		writeHeader(Constants.CAMPS_NAME);
		writeObject(FawkesContextUtil.getAttribute(Constants.CAMPS_NAME, Constants.NON_GROUP));
		// add requestId end
		os.write('m');
	...
	public void startCall(String method, InvocationContext context) throws IOException {
		...
		// add requestId start
		writeHeader(Constants.REQUEST_ID);
		writeString(context.getStrValue(Constants.REQUEST_ID, "0"));
		writeHeader(Constants.GLOBAL_ID);
		writeString(context.getStrValue(Constants.GLOBAL_ID, "0"));
		writeHeader(Constants.TXN_ID);
		writeString(context.getStrValue(Constants.TXN_ID, "0"));
		writeHeader(Constants.INVOKE_TIME);
		writeObject(context.getValue(Constants.INVOKE_TIME, null));
		writeHeader(PropKey.HOST_IP);
		writeObject(PropertiesContainer.getInstance().getProperty(PropKey.HOST_IP, ""));
		writeHeader(PropKeys.APP_ID);
		writeObject(FawkesContextUtil.getAttribute(PropKeys.APP_ID, "unknowPool"));
