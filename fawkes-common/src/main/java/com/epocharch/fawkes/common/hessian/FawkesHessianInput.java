 ...
 * </pre>
 */
public class FawkesHessianInput extends AbstractHessianInput {
	private static Logger logger = LoggerFactory.getLogger(FawkesHessianInput.class);
	 ...
	 *            the underlying input stream.
	 */
	public FawkesHessianInput(InputStream is) {
		init(is);
	}
			...
			if (cl != reader.getType()){
				if(cl.isAssignableFrom(reader.getType())){
					...
					if(logger.isDebugEnabled()) {
						if(!StringUtils.isBlank(type)&&type.contains(".")&&type.startsWith("[")==false) {
							ClassLoader loader = Thread.currentThread().getContextClassLoader();
							String clientAppId = "";
							String clientIp = "";
							try {
								clientAppId = FawkesContextUtil.getString(PropKeys.APP_ID, "");
	...
	public InputStream readInputStream() throws IOException {
		...
		return new InputStream() {
			...
			public int read(byte[] buffer, int offset, int length) throws IOException {
				...
				int len = FawkesHessianInput.this.read(buffer, offset, length);
				if (len < 0)
					_isClosed = true;
