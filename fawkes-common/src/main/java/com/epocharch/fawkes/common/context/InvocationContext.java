...
public class InvocationContext implements Cloneable, Serializable {
	...
	static {
		globalKeyList.put(Constants.GLOBAL_ID, "");
		globalKeyList.put(Constants.REQUEST_HOP, "");
		globalKeyList.put(Constants.TOKEN_GRAY, "");
		globalKeyList.put(Constants.REQUEST_PARENT_ID, "");
}
		...
		if (key != null && value != null) {
			if (value instanceof String) {
				if (value != null) {
					...
					str = FawkesStringUtils.limitString(str, Constants.VALUE_LENGTH_LIMIT);
					value = str;
				}
			}
			if (isGlobalKey(key)) {
	...
	public int getHopValue() {
		int i = -1;
		String v = getStrValue(Constants.REQUEST_HOP, "");
		if (StringUtils.isBlank(v)) {
			v = 1 + "";
			putValue(Constants.REQUEST_HOP, v);
		}
		try {
			i = Integer.valueOf(v);
		} catch (Exception e) {
			...
			i = 1;
			putValue(Constants.REQUEST_HOP, 1 + "");
		}
		return i;
	}

	public void increaseHopValue() {
		putValue(Constants.REQUEST_HOP, getHopValue() + 1);
	}
